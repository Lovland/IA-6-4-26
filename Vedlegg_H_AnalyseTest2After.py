...
df = pd.read_csv(r"C:\Users\danie\OneDrive\Skrivebord\DatasettTest2.csv",
                 header=None,
...
WEIGHT_AT_100_PERCENT_G = 28.62 #29.22 på test 1 pga. tareringsfeil
CUP_WEIGHT_G = 14.61 #15.21 på test 1 pga. tareringsfeil
...
statistics_outliers_removed = df_with_outliers_removed[["WeightDifference", "TimeSpentSeconds", "ErrorPercent"]].describe()
...
def plot_time_distribution(df: pd.DataFrame):
    plt.figure(figsize=(10, 5))
    plt.hist(df['TimeSpentSeconds'], bins=30, edgecolor='black')
    ...
    plt.xticks(np.arange(26, df['TimeSpentSeconds'].max() + 0.5, step=0.5))
